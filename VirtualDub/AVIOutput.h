...
public:
	...
	void setPrimary(BOOL x) { id = x ? mmioFOURCC('0','1','w','b') : mmioFOURCC('0','2','w','b'); }
	BOOL isPrimary() { return id == mmioFOURCC('0','1','w','b'); }
...
protected:
	static char szME[];
public:
	AVIAudioOutputStream	*audio2Out;
	...
	virtual BOOL init(const char *szFile, LONG xSize, LONG ySize, BOOL videoIn, BOOL audioIn, BOOL audio2In, LONG bufferSize, BOOL is_interleaved)=0;
...
private:
	...
	long		audio2_hdr_pos;
	long		audio2_format_pos;
	long		video_hdr_pos;
	long		audio_indx_pos;
	long		audio2_indx_pos;
	long		video_indx_pos;
	long		dmlh_pos;
	long		seghint_pos;
	...
	AVIIndex	*index, *index_audio, *index_audio2, *index_video;
	char *		pHeaderBlock;
	long		nHeaderLen;
	...
	long		lLargestIndexDelta[3];
	__int64		i64FirstIndexedChunk[3];
	__int64		i64LastIndexedChunk[3];
	bool		fLimitTo4Gb;
	long		lIndexedChunkCount[3];
	long		lIndexSize;
	bool		fPreemptiveExtendFailed;

	BOOL		_init(const char *szFile, LONG xSize, LONG ySize, BOOL videoIn, BOOL audioIn, BOOL audio2In, LONG bufferSize, BOOL is_interleaved, bool fThreaded);
	...
	void		_createNewIndices(AVIIndex *index, AVISUPERINDEX *asi, _avisuperindex_entry *asie, AVIAudioOutputStream	*paaos);
...
public:
	...
	BOOL init(const char *szFile, LONG xSize, LONG ySize, BOOL videoIn, BOOL audioIn, BOOL audio2In, LONG bufferSize, BOOL is_interleaved);
