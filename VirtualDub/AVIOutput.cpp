...
BOOL AVIOutputFile::initOutputStreams() {
	if (!(audio2Out = new AVIAudioOutputStream(this))) return FALSE;
	audio2Out->setPrimary(FALSE);
...
BOOL AVIOutputFile::init(const char *szFile, LONG xSize, LONG ySize, BOOL videoIn, BOOL audioIn, BOOL audio2In, LONG bufferSize, BOOL is_interleaved) {
	return _init(szFile, xSize, ySize, videoIn, audioIn, audio2In, bufferSize, is_interleaved, true);
}
...
FastWriteStream *AVIOutputFile::initCapture(const char *szFile, LONG xSize, LONG ySize, BOOL videoIn, BOOL audioIn, LONG bufferSize, BOOL is_interleaved) {
	return _init(szFile, xSize, ySize, videoIn, audioIn, FALSE, bufferSize, is_interleaved, false)
		? fastIO : NULL;
}
...
void AVIOutputFile::writeIndexedChunk(FOURCC ckid, LONG dwIndexFlags, LPVOID lpBuffer, LONG cbBuffer) {
	...
	if (lLargestIndexDelta[2]) {
		idxblocksize = (int)(0x100000000i64 / lLargestIndexDelta[2]);
		if (idxblocksize > MAX_INDEX_ENTRIES)
			idxblocksize = MAX_INDEX_ENTRIES;
		idxblocks += (lIndexedChunkCount[2] + idxblocksize - 1) / idxblocksize;
	}
	...
	} else if ((unsigned short)ckid == '20') {
		if (cbBuffer > audio2Out->streamInfo.dwSuggestedBufferSize) {
			audio2Out->streamInfo.dwSuggestedBufferSize = cbBuffer;
		}

		if (fExtendedAVI)
			if (!index_audio2->add(&avie)) throw MyError("%s error: couldn't add audio chunk to index",szME);
	} else {
...
void AVIOutputFile::_createNewIndices(AVIIndex *index, AVISUPERINDEX *asi, _avisuperindex_entry *asie, AVIAudioOutputStream	*paaos) {
	...
	while(size > 0) {
		...
		actual = _writeNewIndex(&asie[indexnum], asie2, min(size, blocksize),
			paaos ? (paaos->isPrimary()? '10xi' : '20xi') : '00xi', 
			paaos ? paaos->id : videoOut->id, 
			paaos ? paaos->streamInfo.dwSampleSize : videoOut->streamInfo.dwSampleSize);
	...
	asi->dwChunkId		= paaos ? paaos->id : videoOut->id;//is_audio ? 'bw10' : videoOut->id;
}
